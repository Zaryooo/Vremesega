...
const calculateChanceOfRain = (clouds: number, humidity: number, precipitation: number) => {
  let chanceOfRain = 0;
      ...
      <div className='forecast-five-days'>
            ...
            <div className="forecast-body">
              <TabContext value={value}>
                <Box sx={{ borderBottom: 1, borderColor: 'divider', display: 'grid', justifyContent: 'space-between', width: '100%', minWidth: '100%' }}>
                  <TabList onChange={handleChange} aria-label="lab API tabs example" className='w-full grid'>
                    {fiveDaysForecast && fiveDaysForecast.map((item: any) => {
                      const date = `${item.date} ${getMonthInCyrillic(item.mounth)}`;
                      const day = `${item.dayName}`;
                      return item.date && <Tab key={item.date} label={<TabLabel dayName={day} date={date} />} value={item.date} />
                    })}
                  </TabList>
                </Box>
                {fiveDaysForecast && fiveDaysForecast.map((item: any) => {
                  const date = `${item.date} ${getMonthInCyrillic(item.mounth)}`;
                  return item.date && (<TabPanel key={item.date} value={item.date}>
                  <div className='date-hours pt-2' key={item.date}>
                    <div className='hours grid grid-cols-10'>
                      <div className='legend col-span-2'>
                    <div className='date mt-1 pb-3 font-bold'>{date}</div>
                        ...
                        <p className='mt-6'>{item.snow ? `Валежи сняг` : `Количество валежи`}</p>
                        <p className='mt-6'>Облачност</p>
                        <p className='mt-6'>Влажност</p>
                        <p className='mt-6'>Атмосфетно налягане</p>
                      </div>
                    {item.forecast && item.forecast.map((item: any) => {
                      const isDay = city && city.sys ? isDaytime(Number(getHours(item.dt)), Number(getHours(city.sys.sunrise)), Number(getHours(city.sys.sunset))) : true;
                      const rain3h = item.rain && item.rain["3h"] ? item.rain["3h"] : 0;
                      const rain1h = item.rain && item.rain["1h"] ? item.rain["1h"] : 0;
                      const snow3h = item.snow && item.snow["3h"] ? item.snow["3h"] : 0;
                      const snow1h = item.snow && item.snow["1h"] ? item.snow["1h"] : 0;
                      return (<div key={item.dt} className='hour flex flex-col items-center'>
                        <div className='single-hour pt-1'>
                          <p>{`${getHours(item.dt)}:00`}</p>
                        </div>
                        <div className={`weather-icon mt-3 ${item.weather[0].description.toLowerCase()}`} aria-label={`${item.weather[0].main.toLowerCase()} icon`} style={{ backgroundPosition: getWeatherIconPosition(item.weather[0].description.toLowerCase(), isDay) }}></div>
                        <div className={`weather mt-3 mb-6 flex items-center text-center text-[14px] leading-[16px] min-h-[34px] ${item.weather[0].description.toLowerCase()}`} aria-label={`${item.weather[0].main.toLowerCase()}`} ><p>{`${getWeatherCondition(item.weather[0].description.toLowerCase(), isDay) }`}</p></div>
                        <div className='temp mb-9'>
                          <span className='font-bold'>{Math.round(item.main.temp_max)}°</span>
                        </div>
                        <div className='wind mb-6'>
                          <span className=''>{Math.round(item.wind.speed)} m/s</span>
                        </div>
                        <div className='rain-chance mb-6'>
                          <span className=''>{Math.round(calculateChanceOfRain(item.clouds.all, item.humidity,rain3h || rain1h))} %</span>
                        </div>
                        <div className='rain mb-6'>
                          <span className=''>{(item.rain && rain3h || rain1h) || (item.snow && snow3h || snow1h)} mm</span>
                        </div>
                        ...
                        <div className='pressure mb-6'>
                          <span className=''>{item.main.pressure} hPa</span>
                        </div>
                      </div>)
                })}
                    </div>
